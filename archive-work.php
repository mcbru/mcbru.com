...
get_header(); ?>
			...
			<div class="twenty-four columns">
				...
				<div class="tucker">
					...
					<ul class="horz-subnav">
						<li class="curr"><a href="javascript:alert('naw juss yet loov');">All Work</a></li>
		...
		<?php if ( have_posts() ) : ?>
			<?php while ( have_posts() ) : the_post(); $d = $post->ID; $cs = get_field('rel_cs',$d); $subt = get_field('subtitle',$d); ?>
						...
						<?php //for($i=0;$i<12;$i++) : ?>
							<a class="tile">
								<?php if(has_post_thumbnail($d)) : $img = wp_get_attachment_image_src( get_post_thumbnail_id($d), 'full' );//$img[0] ?>
